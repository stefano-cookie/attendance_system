...
const AttendancePanel: React.FC = () => {
  ...
  // Stati per le opzioni di selezione
  ...
  // Ricarica presenze quando cambiano i filtri principali
  useEffect(() => {
    fetchAttendanceData();
  ...
  const loadInitialData = async () => {
    try {
      // Carica corsi, materie e lezioni in parallelo
  ...
  const fetchAttendanceData = useCallback(async () => {
    try {
      ...
      // Formatta le date per il backend
      ...
      // Se c'è un filtro per lezione specifica, usa l'endpoint complete
      if (filters.lessonId) {
            ...
            setStats({
              ...
              attendanceByLesson: {
                [filters.lessonId]: {
                  ...
                  date: new Date(lessonInfo.lesson_date).toLocaleDateString('it-IT'),
        ...
        response.data.attendancesByLesson.forEach((lessonData: any) => {
          ...
          lessonStats[lessonData.lesson.id] = {
            ...
            date: new Date(lessonData.lesson.date).toLocaleDateString('it-IT'),
      ...
      setError('Errore nel caricamento dei dati delle presenze.');
    } finally {
      setLoading(false);
    }
  ...
  const exportToExcel = () => {
    const worksheetData = [
      ['Report Presenze - Sistema di Rilevamento Presenze'],
      ['Generato il', new Date().toLocaleDateString('it-IT') + ' ' + new Date().toLocaleTimeString('it-IT')],
      [''],
      ['RIEPILOGO GENERALE'],
      ['Totale Studenti', stats?.totalStudents || 0],
      ['Totale Lezioni', stats?.totalLessons || 0],
      ['Totale Presenze', stats?.presentStudents || 0],
      ['Totale Assenze', stats?.absentStudents || 0],
      ['Percentuale Media Presenze', `${(stats?.averageAttendance || 0).toFixed(2)}%`],
      [''],
      ['DETTAGLIO PRESENZE'],
      ['Studente', 'Matricola', 'Lezione', 'Data', 'Stato', 'Affidabilità %', 'Timestamp']
    ];
    
    filteredAttendances.forEach(att => {
      worksheetData.push([
        `${att.student.name} ${att.student.surname}`,
        att.student.matricola || 'N/A',
        att.lesson?.name || 'N/A',
        att.lesson?.lesson_date ? new Date(att.lesson.lesson_date).toLocaleDateString('it-IT') : 'N/A',
        att.is_present ? 'Presente' : 'Assente',
        att.is_present && att.confidence > 0 ? `${(att.confidence * 100).toFixed(0)}%` : '-',
        att.timestamp ? new Date(att.timestamp).toLocaleString('it-IT') : '-'
      ]);
    });
    ...
    if (stats?.attendanceByLesson && Object.keys(stats.attendanceByLesson).length > 0) {
      const lessonStatsData = [
        ...
        ['Lezione', 'Data', 'Presenti', 'Assenti', 'Totale', 'Percentuale Presenze']
      ];
      ...
      XLSX.utils.book_append_sheet(wb, wsLessons, 'Statistiche Lezioni');
    }
    
    const excelBuffer = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
    const data = new Blob([excelBuffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
    
    const fileName = `presenze_${filters.lessonId ? `lezione_${filters.lessonId}_` : ''}${new Date().toISOString().split('T')[0]}.xlsx`;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-100 flex justify-center items-center">
        <div className="text-center bg-white p-8 rounded-2xl shadow-xl">
          ...
          <h3 className="text-xl font-semibold text-gray-700">Caricamento Presenze</h3>
          <p className="text-gray-500 mt-2">Analisi dei dati in corso...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex flex-col md:flex-row justify-between items-center">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-800">Gestione Presenze</h1>
                <p className="text-gray-600">Monitoraggio e Analisi delle Presenze</p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-3">
              ...
              >
                ...
                <span className="font-semibold">Esporta Excel</span>
              </button>
            </div>
          </div>
        </div>
        ...
        {stats && (
          <div className="grid grid-cols-1 md:grid-cols-5 gap-6 mb-8">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100 hover:shadow-xl transition-shadow">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600 mb-1">Lezioni</p>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100 hover:shadow-xl transition-shadow">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600 mb-1">Presenze</p>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100 hover:shadow-xl transition-shadow">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600 mb-1">Assenze</p>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 mb-8">
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
              {/* Filtro Corso */}
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Corso</label>
                <select
              ...
              {/* Filtro Lezione */}
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Lezione Specifica</label>
                <select
                  ...
                  {lessons
                    ...
                    .map(lesson => (
                      <option key={lesson.id} value={lesson.id}>
                        {lesson.name} - {lesson.course?.name || 'N/A'} ({new Date(lesson.lesson_date).toLocaleDateString('it-IT')})
                      </option>
                    ))}
                </select>
              </div>
              ...
              {/* Ricerca Studente */}
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Cerca Studente</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Nome, cognome o matricola"
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              {/* Filtri Stato */}
              <div className="md:col-span-2">
                <label className="block text-sm font-semibold text-gray-700 mb-2">Mostra Solo</label>
                <div className="flex space-x-6">
                  <label className="inline-flex items-center">
                    ...
                    />
                    <span className="ml-2 text-sm font-medium text-gray-700">Solo Presenti</span>
                  </label>
                  <label className="inline-flex items-center">
                    <input
                      type="checkbox"
                    ...
                    />
                    <span className="ml-2 text-sm font-medium text-gray-700">Solo Assenti</span>
                  </label>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <h3 className="text-xl font-semibold text-gray-800">
                  {viewMode === 'list' && 'Elenco Dettagliato'}
                  {viewMode === 'grid' && 'Vista a Griglia'}
                  {viewMode === 'stats' && 'Analisi Statistiche'}
                </h3>
              </div>
              <span className="bg-gray-100 text-gray-600 px-3 py-1 rounded-full text-sm font-medium">
                {filteredAttendances.length} {filteredAttendances.length === 1 ? 'record' : 'records'}
              </span>
            </div>
          </div>
              ...
              <>
                {filteredAttendances.length > 0 ? (
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {filteredAttendances.map((attendance, index) => (
                          ...
                          >
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm text-gray-900">{attendance.student.matricola || 'N/A'}</div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm text-gray-900">
                                {attendance.lesson?.name || 'N/A'}
                              </div>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm text-gray-500">
                                {attendance.lesson?.lesson_date 
                                  ? new Date(attendance.lesson.lesson_date).toLocaleDateString('it-IT')
                                  : 'N/A'
                                }
                              </div>
                            </td>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                              {attendance.timestamp 
                                ? new Date(attendance.timestamp).toLocaleTimeString('it-IT', { 
                                    hour: '2-digit', 
                                    minute: '2-digit' 
                                  })
                                : '-'
                  ...
                  <div className="text-center py-16">
                    ...
                    <h3 className="text-xl font-semibold text-gray-900 mb-2">Nessun dato di presenza trovato</h3>
                    <p className="text-gray-500 mb-8">
                      Non ci sono dati di presenza che corrispondono ai filtri applicati
                    </p>
                    <button
                      onClick={resetFilters}
              ...
              <>
                {filteredAttendances.length > 0 ? (
                        ...
                        <div className="p-6">
                          <div className="flex justify-between items-start mb-4">
                            <div className="flex items-center">
                              ...
                              <div className="ml-3">
                                ...
                                <p className="text-xs text-gray-500">{attendance.student.matricola || 'N/A'}</p>
                              </div>
                            </div>
                          ...
                          <div className="space-y-3">
                            <div className="flex items-center p-3 bg-blue-50 rounded-lg">
                              ...
                              <div>
                                <p className="text-xs font-medium text-gray-700">Lezione</p>
                            ...
                            <div className="flex items-center p-3 bg-purple-50 rounded-lg">
                              ...
                              <div>
                                <p className="text-xs font-medium text-gray-700">Data</p>
                            ...
                            {attendance.is_present && attendance.confidence > 0 && (
                              <div className="flex items-center p-3 bg-green-50 rounded-lg">
                                ...
                                <div className="flex-1">
                                  <p className="text-xs font-medium text-gray-700">Affidabilità</p>
                                  <div className="flex items-center">
                            ...
                            {attendance.timestamp && (
                              <div className="flex items-center p-3 bg-gray-50 rounded-lg">
                                ...
                                <div>
                                  <p className="text-xs font-medium text-gray-700">Rilevato alle</p>
                  ...
                  <div className="text-center py-16">
                    ...
                    <h3 className="text-xl font-semibold text-gray-900 mb-2">Nessun dato nella griglia</h3>
                    <p className="text-gray-500 mb-8">
                      Non ci sono dati di presenza da visualizzare nella griglia
                    </p>
                    <button
                      onClick={resetFilters}
                      className="inline-flex items-center px-6 py-3 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-lg shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-200 font-semibold"
                ...
                {Object.keys(stats.attendanceByLesson).length > 0 && (
                  <div className="bg-gradient-to-br from-white to-blue-50 p-6 rounded-2xl shadow-lg border border-blue-100">
                    <div className="flex items-center space-x-3 mb-6">
                      ...
                      <h3 className="text-xl font-semibold text-gray-800">Statistiche per Lezione</h3>
                    </div>
                        ...
                        <div key={lessonId} className="bg-white p-6 rounded-xl shadow-md border border-gray-200 hover:shadow-lg transition-shadow">
                          <div className="flex justify-between items-start mb-4">
                            ...
                            <div className="text-right">
                              ...
                              <p className="text-sm text-gray-500">
                                {data.presentCount}/{data.totalCount} presenti
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div className="text-center p-3 bg-green-50 rounded-lg">
                              ...
                              <p className="text-xs text-green-700 font-medium">Presenti</p>
                            </div>
                            <div className="text-center p-3 bg-red-50 rounded-lg">
                              <p className="text-2xl font-bold text-red-600">{data.absentCount}</p>
                              <p className="text-xs text-red-700 font-medium">Assenti</p>
                            </div>
                          </div>
                        </div>
                      ))}
                ...
                {Object.keys(stats.attendanceByStudent).length > 0 && (
                  <div className="bg-gradient-to-br from-white to-green-50 p-6 rounded-2xl shadow-lg border border-green-100">
                    <div className="flex items-center space-x-3 mb-6">
                      ...
                      <h3 className="text-xl font-semibold text-gray-800">Statistiche per Studente</h3>
                    </div>
